...
public class CustomPinterestTV extends ViewGroup {
    ...
    public static final int MODE_AUTO_SCROLL_STANDARD = 5;

    public static final int MODE_AUTO_SCROLL_FLING = 6;
    ...
    private static final int DURATION_BUFFER = 100;

    private static final int TV_WALK_DURATION = 450;

    private static final int TV_BIKE_DURATION = 280;

    private static final int TV_TRAIN_DURATION = 150;
    ...
    private int mDPadClickCount = 0;

    private static final int AUTO_SCROLL_FLING = 150;

    private static final int AUTO_SCROLL_FAST = 80;

    private static final int AUTO_SCROLL_STANDARD = 40;

    private int mItemEdgeViewId = -1;

    private View mItemEdgeView;

    private static final long TOTAL_SCROLL_TIME = Long.MAX_VALUE;

    private LinkedList<Integer> mScrollCount;

    public CustomPinterestTV(Context context) {
        super(context);
        init();
    }

    public CustomPinterestTV(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init();
    }

    public CustomPinterestTV(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private class AutoScroll implements Runnable {
        ...
        public void run() {
            ...
            directScrollTVPin(mAutoScrollSpeed);
            if (mIsAutoScroll && System.currentTimeMillis() - mTime < TV_WALK_DURATION) {
                CustomPinterestTV.this.postDelayed(new AutoScroll(), 10);
            } else {
                dealAutoScrollEdgeStop();
            }
        }
    }

    private void dealAutoScrollEdgeStop() {
        mIsAutoScroll = false;
        mDPadClickCount = 0;
        ...
        if (mAutoScrollSpeed < 0) {
            int last = mListItemPosition[mFoucsColumn].getLast();
            view = mCurrentViews.get(last);
            if (view.getBottom() + mSpaceVertical >= getHeight()) {
                // view.requestFocus();
                int verticalDistance = getHeight() - view.getBottom() - mSpaceVertical;
                Log.i(TAG, "DPadDown need scroll down distance  " + verticalDistance);
                scrollTVPin(verticalDistance, 400);
                mPreDuration = 400 + DURATION_BUFFER;
            }
            mFocusPosition = last;
        } else {
            int first = mListItemPosition[mFoucsColumn].getFirst();
            view = mCurrentViews.get(first);
            if (view.getTop() < 0) {
                int verticalDistance = 0;
                if (mFocusPosition >= mColumn) {
                    verticalDistance += mSpaceVertical;
                }
                verticalDistance -= view.getTop();
                scrollTVPin(verticalDistance, 400);
                mPreDuration = 400 + DURATION_BUFFER;
            } else if (mFocusPosition < mColumn) {
                Log.e(TAG, "animation top edge");
                Animation animation = new TranslateAnimation(view.getLeft(), view.getLeft(), view.getTop(), view.getTop());
                animation.setFillAfter(true);
                animation.setDuration(100);
                mPreDuration = 100 + DURATION_BUFFER;
                mItemEdgeView.startAnimation(animation);
            }
            mFocusPosition = first;
            view.requestFocus();
        }
    }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (mPreDPadPressed < 0) {
            mPreDPadPressed = code;
        }
        ...
        case KeyEvent.ACTION_DOWN:
            timeGap = System.currentTimeMillis() - mTime;
            ...
            if (repeatCount > 1) {
                if (code != KeyEvent.KEYCODE_DPAD_UP && code != KeyEvent.KEYCODE_DPAD_DOWN) {
                    mIsAutoScroll = false;
                    mPreDuration = TV_ROCKET_DURATION - 30;
                    duration = TV_ROCKET_DURATION - 30;
                } else if (repeatCount == 2) {
                    // duration = (int) TV_TRAIN_DURATION;
                    mAutoScrollSpeed = AUTO_SCROLL_FLING;
                    mTouchMode = MODE_AUTO_SCROLL_FLING;
                    mDPadClickCount++;
                }
            }
            Log.i(TAG_COUNT, "down dpad timeGap = " + timeGap);
            if (repeatCount < 1) {
                if (timeGap > TV_WALK_DURATION) {
                    if (!mIsAutoScroll) {
                        if (code != KeyEvent.KEYCODE_DPAD_UP && code != KeyEvent.KEYCODE_DPAD_DOWN || code != mPreDPadPressed) {
                            duration = TV_WALK_DURATION - 2 * DURATION_BUFFER;
                            mPreDuration = duration;
                        } else {
                            mPreDuration = TV_WALK_DURATION - DURATION_BUFFER;
                            mDPadClickCount = 0;
                            mIsAutoScroll = false;
                            duration = (int) mPreDuration;
                        }
                    }

                } else if (timeGap > TV_WALK_DURATION - DURATION_BUFFER) {
                    if (!mIsAutoScroll) {
                        if (code != KeyEvent.KEYCODE_DPAD_UP && code != KeyEvent.KEYCODE_DPAD_DOWN || code != mPreDPadPressed) {
                            duration = TV_WALK_DURATION - 2 * DURATION_BUFFER;
                            mPreDuration = duration;
                            ...
                            mTouchMode = MODE_AUTO_SCROLL_STANDARD;
                            mAutoScrollSpeed = AUTO_SCROLL_STANDARD;
                            mDPadClickCount++;
                        }
                    }
                ...
                } else if (timeGap > TV_BIKE_DURATION - DURATION_BUFFER) {
                    if (!mIsAutoScroll) {
                        if (code != KeyEvent.KEYCODE_DPAD_UP && code != KeyEvent.KEYCODE_DPAD_DOWN || code != mPreDPadPressed) {
                            mPreDuration = TV_BIKE_DURATION - DURATION_BUFFER;
                            duration = TV_BIKE_DURATION - DURATION_BUFFER;
                        } else {
                            duration = TV_BIKE_DURATION - DURATION_BUFFER;
                            mTouchMode = MODE_AUTO_SCROLL_STANDARD;
                    ...
                    if (code != KeyEvent.KEYCODE_DPAD_UP && code != KeyEvent.KEYCODE_DPAD_DOWN || code != mPreDPadPressed) {
                        mPreDuration = TV_ROCKET_DURATION;
                        duration = TV_ROCKET_DURATION;
                    } else {
                        duration = TV_TRAIN_DURATION - DURATION_BUFFER;
                        // duration = (int) TV_TRAIN_DURATION;
                        mTouchMode = MODE_AUTO_SCROLL_STANDARD;
                        mAutoScrollSpeed = AUTO_SCROLL_FAST;
                        mDPadClickCount++;
                    }
                }
            }
            ...
            // if (code != KeyEvent.KEYCODE_DPAD_UP && code !=
            ...
            case KeyEvent.KEYCODE_DPAD_DOWN:
                dealDPadDown(duration, code);
                break;
            case KeyEvent.KEYCODE_DPAD_CENTER:
                break;
            }
            if (!mIsAutoScroll && (repeatCount == 2 || mDPadClickCount == 1) && (code == KeyEvent.KEYCODE_DPAD_DOWN || code == KeyEvent.KEYCODE_DPAD_UP)
                    && code == mPreDPadPressed) {
                Log.e(TAG_SCROLL, "postOnAnimation()!");
                mIsAutoScroll = true;
                post(new AutoScroll());
            }
            mPreDPadPressed = code;
    ...
    private boolean isAutoScrolling() {
        return mTouchMode == MODE_AUTO_SCROLL_STANDARD || mTouchMode == MODE_AUTO_SCROLL_FLING;
    }

    private void dealDPadUp(int duration, int code) {
        if (isAutoScrolling() || mIsAutoScroll && mPreDPadPressed == code) {
            mAutoScrollSpeed = Math.abs(mAutoScrollSpeed);
        } else if (mFocusPosInList > 0) {
            ...
            if (nextView.getTop() - mSpaceVertical < 0) {
                if (mFocusPosition >= mColumn) {
                    verticalDistance += mSpaceVertical;
                }
    ...
    private void dealDPadDown(int duration, int code) {
        ...
        if (isAutoScrolling() || mIsAutoScroll && mPreDPadPressed == code) {
            mAutoScrollSpeed = -Math.abs(mAutoScrollSpeed);
        } else if (mFocusPosInList < mListItemPosition[mFoucsColumn].size() - 1) {
            int verticalDistance = 0;
            View view = mCurrentViews.get(mListItemPosition[mFoucsColumn].get(mFocusPosInList));
    ...
    private void dealDPadRight(int duration) {
        ...
        if (mIsAutoScroll || isAutoScrolling()) {
            stopAutoScroll();
        } else {
    ...
    private void directScrollTVPin(int distance) {
        directScrollPin(distance);
        calculatePin();
        postInvalidate();
    }
    ...
    private void pullUp(int[] firstBottom) {
        // add item at the bottom
        {
            ...
            if (mLastItemPos == mItemCount - 1) {
                ...
                if (mPinBottom[j] + mScrollDistance < getHeight()) {
                    Log.i(TAG, "origin : " + mScrollDistance + "real distance : " + (-Math.abs(mPinBottom[j] - getHeight())));
                    mScrollDistance = getHeight() - mPinBottom[j];
                    if (mPinterestCallBack != null) {
                        mPinterestCallBack.loadMore();
                    }
                    mScroller.forceFinished(true);
