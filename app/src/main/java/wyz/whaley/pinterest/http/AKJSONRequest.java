...
public class AKJSONRequest extends AKRequest {

    private static final String TAG = "JSONRequest";

    private int sum = 0;

    public AKJSONRequest(String url, AKResponseListener listener, int requestMode, Context context) {
    ...
    public void dispatcherResult(InputStream in) {
        ...
        if (this.mListener != null) {
            this.mListener.onSuccess(praseJsonArray(in));
        }
    }
    ...
    private List<ItemInfo> praseJsonArray(InputStream inputStream) {
        ...
        try {
            ...
            SharedPreferences sharedPreferences = mContext.getSharedPreferences(SharedPreferenceUtils.PREFERRENCE_SETTING, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putInt(SharedPreferenceUtils.ITEM_SUM, sum).commit();
